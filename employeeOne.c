...
PtrToEmployee searchEmployeeByPhoneNumber(PtrToConstEmployee ptr, int tableSize, char * targetPhoneNum)
{
    const PtrToConstEmployee endPtr = ptr + tableSize; 
    for(; ptr < endPtr; ptr++) 
    { 
        if(strcmp(ptr->phone,targetPhoneNum) == 0) 
            return (PtrToEmployee) ptr; 
    } 
    return NULL; 
}

PtrToEmployee searchEmployeeBySalary(PtrToConstEmployee ptr, int sizeTable, double targetSalary)
{
    const PtrToConstEmployee endPtr = ptr + sizeTable; 
    for(; ptr < endPtr; ptr++)
    { 
